...
class ROTIMap:
  ...
  def read_rotimap(self, bytes_obj: bytes) -> None:
    ...
    for _, group in groupby(lines[start_rotimap + 2:end_rotimap],
  ...
  def plot(self) -> str:
    ...
    cax = ax.contourf(theta, r, self.ROTI, levels=np.linspace(0, 1, 51), cmap="jet", vmin=0, vmax=1)
    cbar = fig.colorbar(cax, ticks=np.linspace(0, 1, 6), orientation="horizontal", pad=0.1)
