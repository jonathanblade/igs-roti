...
class ROTIMapDB:
    def __init__(self, user: str, password: str):
        client = MongoClient(
            f"mongodb+srv://{user}:{password}@Cluster0.9nhgn.mongodb.net/igs_roti_db?retryWrites=true&w=majority"
        )
    ...
    def save_map(self, date: Union[str, datetime.date], png_data: str) -> None:
        self.collection.insert_one({"date": self.date2str(date), "png_data": png_data})

    def get_map(self, date: Union[int, datetime.date]) -> Optional[dict]:
        return self.collection.find_one(
            {"date": self.date2str(date)}, {"png_data": 1, "_id": 0}
        )

    def get_dates(self) -> List[dict]:
        return list(
            self.collection.find({}, {"date": 1, "_id": 0}).sort("date", ASCENDING)
        )
